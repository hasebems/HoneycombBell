...
//  Serial.begin(31250);
  ...
  pinMode(5, INPUT); 
  pinMode(6, INPUT);
  pinMode(MASTER_NORMAL, INPUT);
  const uint8_t setupJmpr = digitalRead(3);
  const uint8_t whichMbrJmpr = digitalRead(4);
  ...
  //  Check who I am
  int setNumber;
  const uint8_t dip1 = digitalRead(4);
  const uint8_t dip2 = digitalRead(5);
  const uint8_t dip3 = digitalRead(6);
